...
void SetCursor(void)
{
    ...
    x = 150 + (CurPos)*23;
    if (x > 23)
        LCD_FillRect((LCDPoint){x - 23, 32}, (LCDPoint){x + 46, 35}, LCD_BLACK); //delete the old cursor
    else
        LCD_FillRect((LCDPoint){x, 32}, (LCDPoint){x + 46, 35}, LCD_BLACK); //delete the old cursor
...
static const TEXTBOX_t tb_keybal[] = {
    ...
    (TEXTBOX_t){.x0 = KBDX(4), .y0 = KBDY(4) + 4, .text = "<", .font = FONT_FRANBIG, .width = KEYW, .height = 30, .center = 1, .border = 1, .fgcolor = LCD_WHITE, .bgcolor = LCD_GRAY, .cb = KeybHitLeftCb, .next = (void *)&tb_keybal[39]},
    (TEXTBOX_t){.x0 = KBDX(6), .y0 = KBDY(4) + 4, .text = ">", .font = FONT_FRANBIG, .width = KEYW, .height = 30, .center = 1, .border = 1, .fgcolor = LCD_WHITE, .bgcolor = LCD_GRAY, .cb = KeybHitRightCb, .next = (void *)&tb_keybal[40]},
    (TEXTBOX_t){.x0 = KBDX(0), .y0 = KBDY(4) + 4, .text = "Cancel", .font = FONT_FRANBIG, .border = 1, .center = 1, .width = 90, .height = 32, .fgcolor = LCD_BLUE, .bgcolor = LCD_YELLOW, .cb = KeybHitCancelCb, .next = (void *)&tb_keybal[41]},
    (TEXTBOX_t){
        .x0 = KBDX(7),
        .y0 = KBDY(4) + 4,
        .text = "OK",
