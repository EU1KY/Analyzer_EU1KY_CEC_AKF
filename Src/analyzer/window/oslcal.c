...
void OSL_CalWnd(void)
{
    ...
    hbEx = (TEXTBOX_t){.x0 = 10, .y0 = 220, .text = " Cancel and exit ", .font = FONT_FRANBIG, .fgcolor = LCD_BLUE, .bgcolor = LCD_YELLOW, .cb = _hit_ex};
    hbScanShort = (TEXTBOX_t){.x0 = 10, .y0 = 50, .text = shortvalue, .font = FONT_FRANBIG, .fgcolor = LCD_BLUE, .bgcolor = LCD_RGB(64, 64, 64), .cb = _hb_scan_short};
    hbScanLoad = (TEXTBOX_t){.x0 = 10, .y0 = 100, .text = loadvalue, .font = FONT_FRANBIG, .fgcolor = LCD_BLUE, .bgcolor = LCD_RGB(64, 64, 64), .cb = _hb_scan_load};
    hbScanOpen = (TEXTBOX_t){.x0 = 10, .y0 = 150, .text = openvalue, .font = FONT_FRANBIG, .fgcolor = LCD_BLUE, .bgcolor = LCD_RGB(64, 64, 64), .cb = _hb_scan_open};
    hbSave = (TEXTBOX_t){.x0 = 300, .y0 = 220, .text = " Save and exit ", .font = FONT_FRANBIG, .fgcolor = LCD_RGB(128, 128, 128), .bgcolor = LCD_RGB(64, 64, 64), .cb = _hit_save};
...
void OSL_CalErrCorr(void)
{
    ...
    TEXTBOX_t hbEx = {.x0 = 10, .y0 = 220, .text = " Exit ", .font = FONT_FRANBIG, .fgcolor = LCD_BLUE, .bgcolor = LCD_YELLOW, .cb = _hit_ex};
    TEXTBOX_Append(&osl_ctx, &hbEx);

    //Reusing hbScanShort
    hbScanShort = (TEXTBOX_t){.x0 = 100, .y0 = 120, .text = " Start HW calibration ", .font = FONT_FRANBIG, .fgcolor = LCD_RED, .bgcolor = LCD_RGB(64, 64, 64), .cb = _hit_err_scan};
...
{
    if (progress == 1)
        SaveS21CorrToFile(); // save without attenuator values
    rq21Exit = 1;
}
...
static TEXTBOX_t tb_S21_CALIBRATION[] =
    {
        ...
        (TEXTBOX_t){.x0 = 0, .y0 = 110, .text = "(1) S21 calibration without attenuator", .font = FONT_FRANBIG, .width = 476, .height = 34, .center = 1, .border = 1, .fgcolor = LCD_RED, .bgcolor = LCD_RGB(64, 64, 64), .cb = _hit_tx_scan, .cbparam = 1, .next = (void *)&tb_S21_CALIBRATION[2]},
        (TEXTBOX_t){.x0 = 0, .y0 = 160, .text = "(2) S21 calibration with Attenuator", .font = FONT_FRANBIG, .width = 476, .height = 34, .center = 1, .border = 1, .fgcolor = LCD_RED, .bgcolor = LCD_RGB(64, 64, 64), .cb = _hit_att_scan, .cbparam = 1, .next = (void *)&tb_S21_CALIBRATION[3]},
        (TEXTBOX_t){
            .border = 1,
            .cbparam = 1,
            .center = 1,
